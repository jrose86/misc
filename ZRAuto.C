// AUTOMATED version of ZeroRepulsion bc im a silly silly goose

// idea is to create graphs like in TauBranching - look at window of Tau vs set parameter Tau
// COMMENT EITHER TAU START OR TAU WINDOW
...
void ZRAuto(){
  ...
  double signalAmp(0.5),signalSig(4.0),signalPos(10.);
  int ntries=5000;

  int nAvg(5); // number of data pts to avg for each final point

  //============================== Comment one of the chunks below ==========================================

  int nStartTau = 21;
  double pcBranched[nStartTau];
  double pcBranchedNF[nStartTau];
  double pcBtemp[nAvg];
  double pcBtempNF[nAvg];
  double Tstart = 0.0;
  double minDelTau = 0.0;
  double maxDelTau = 20.0;

  for (int is = 0; is<nStartTau; is++){
    double tempSum(0);
    double tempSumNF(0);
    for (int q=0; q<nAvg; q++){


  //----------------------------------------------------------------------------------------------------------
      /*
  int nDeltaTau = 26;
  double pcBranched[nDeltaTau];
  double pcBranchedNF[nDeltaTau];
  double pcBtemp[nAvg];
  double pcBtempNF[nAvg];
  double minDelTau = signalPos-1.0;
  double maxDelTau = signalPos+1.0;
  double Tstart;

  for (int is = 0; is<nDeltaTau; is++){
    if (maxDelTau-minDelTau > 20.0){  Tstart = 0.0;}
    else {Tstart = minDelTau +1.0;}
  ...
  myGaus->SetParName(1,"#tau_{0}");   /*myGaus->FixParameter(1,5.);*/       myGaus->SetParLimits(1,minDelTau, maxDelTau); // <------- Josie, please play with these Limits. (orig: -100,100)
  ...
  //double signalAmp(2.0),signalSig(4.0),signalPos(10.);
  //int ntries=5000;
  int numFarOff(0);
  int numFlag1(0);
  int numFlag2(0);
  ...
  for (int i=0; i<ntries; i++){
    ...
    myGaus->SetParameter(1,Tstart);    // <--- THIS was causing the problems where we didn't see the branching at 0 amp
    int Flags = 0;
    if (i<10){
      tc->cd(i+1);
      CFsim->Fit(myGaus);
 ...
 // tc2->Draw();
  ...
  pcBtemp[q] = double(numFarOff)/ntries*100.0;
  pcBtempNF[q] = double(numFarNotFlagged)/(ntries-numFlag2)*100.0;
  tempSum += pcBtemp[q];
  tempSumNF += pcBtempNF[q];
    } // <--- end of for loop for taking avg of nAvg pts
    pcBranched[is] = tempSum/nAvg;
    pcBranchedNF[is] = tempSumNF/nAvg;
  ...
  //---------------------------------------------------------------------------------------
    /*
  if (maxDelTau-minDelTau < 30.0){
    minDelTau += -1.0;
    maxDelTau +=  1.0;
  }
  else if(maxDelTau-minDelTau < 100.0){
    minDelTau += -5.0;
    maxDelTau +=  5.0;
  }
  else if(maxDelTau-minDelTau < 160.0){
    minDelTau += -10.0;
    maxDelTau +=  10.0;
  }
  else{
  ...
  /* pcBranched[is] = numFarOff*100.0;
  pcBranchedNF[is] = double(numFarNotFlagged)/(ntries-numFlag2)*100.0;
  delTau[is] = maxDelTau-minDelTau;
  */
  } // <--- end of for loop for making 1 pcB data pt averaged

  cout << "%B array: " << endl;
  //======= Comment one of these! =========================================================
  for (int in=0; in<nStartTau; in++){
    cout << pcBranched[in] << ", ";
    }
   cout << endl;
   cout << "%B flags cut: ";
    for (int im=0; im<nStartTau; im++){
    cout << pcBranchedNF[im] << ", ";
    }

  //---------------------------------------------------------------------------------------
    /*
   for (int in=0; in<nDeltaTau; in++){
     cout << pcBranched[in] << ", ";
   }
   cout << endl;
   cout << "%B flags cut: ";
   for (int im=0; im<nDeltaTau; im++){
     cout << pcBranchedNF[im] << ", ";
     }*/
